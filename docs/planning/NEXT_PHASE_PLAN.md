...
- **P1 前端**: 约 2-3 小时 (UI 重构较复杂，特别是递归渲染和交互细节)
- **P2 全栈**: 约 2 小时 (相对标准的 CRUD)

---

## P3: 功能增强 (New Features)

### 1. 🔗 友链/导航系统 (External Links)

**目标**：提供一个管理和展示"喜欢的网站"或"有意思的资源"的页面。

- **功能描述**:
  - 后台 CRUD：添加网站名称、URL、Logo、描述、分类（如：技术、设计、娱乐）。
  - 前台展示：卡片式布局展示导航列表。
- **实现方案**:
  - 新增 `ExternalLink` 实体。
  - Admin 管理页面 + 前台 `/links` 页面。

### 2. 🗓️ 纪念日行程规划 (Anniversary Itinerary)

**目标**：为特定纪念日制定私密行程或计划。

- **功能描述**:
  - **管理员可见**：行程内容仅在后台或登录后的 UI 显示，普通访客不可见。
  - 关联纪念日：每个行程项关联到一个具体的 `Anniversary`。
- **实现方案**:
  - 新增 `ItineraryItem` 实体 (Time, Content, Location, AnniversaryId)。
  - 在纪念日详情页展示时间轴行程。
