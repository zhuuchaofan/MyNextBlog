...
| **CommentService**             | `ICommentService`             | 触发器。在评论创建流程中触发邮件通知。                                                |
...
4.  **作用域创建**: 创建新的 `IServiceScope` 以解析 Service (因 `DbContext` 是 Scoped 的)。
5.  **数据加载**: `SendNotificationsAsync` 内部使用 `Include` 一次性加载关联数据 (Post, User, ParentComment)。
6.  **逻辑判断**:
