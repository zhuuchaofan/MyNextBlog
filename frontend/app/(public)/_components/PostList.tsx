...
export default function PostList({
  ...
  const loadMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/backend/posts?page=${nextPage}&pageSize=10`
      );
  ...
  const toggleVisibility = async (e: React.MouseEvent, post: Post) => {
    ...
    // 乐观更新
    setPosts(
      posts.map((p) => (p.id === post.id ? { ...p, isHidden: !p.isHidden } : p))
    );
    ...
    <div className="space-y-8">
      <div className="space-y-6">
        {posts.map((post) => (
          <Card
            key={post.id}
            className={`group overflow-hidden border border-gray-100 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 rounded-3xl ${
              post.isHidden
                ? "opacity-70 grayscale-[0.5] border-dashed border-gray-300"
                : ""
            }`}
          >
              ...
              <div className="flex-1 flex flex-col p-6 md:p-8">
                ...
                <div className="flex items-center gap-2 mb-4 min-w-0 overflow-hidden">
                  {post.isHidden && (
                    <Badge
                      variant="destructive"
                      className="h-6 px-2 text-xs border-dashed border-red-300 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400"
                    >
                      Hidden
                    </Badge>
                  )}
                  ...
                  <Link href={`/categories/${post.categoryId}`}>
                    <Badge
                      variant="secondary"
                      className="bg-orange-50 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/50 rounded-lg px-3 py-1 transition-colors"
                    >
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {post.tags.slice(0, 4).map((tag) => (
                      ...
                      >
                        <Badge
                          variant="outline"
                          className="text-xs text-gray-500 dark:text-gray-400 border-gray-200 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800 cursor-pointer rounded-md px-2 py-0.5 transition-colors"
                        >
                          # {tag}
                        </Badge>
                      </Link>
                    ...
                    {post.tags.length > 4 && (
                      <Badge
                        variant="outline"
                        className="text-xs text-gray-400 dark:text-gray-500 border-gray-200 dark:border-zinc-700 rounded-md px-2 py-0.5"
                      >
                        +{post.tags.length - 4}
                      </Badge>
                    )}
